...
对于静态分析使用无限单元时，必须定位极点，以为正在建模的特定问题提供合理的远场解。Abaqus中的无限单元编写时，在有限和无限单元之间的界面以及在每个延伸到无穷远的边上有一个节点，该节点必须放置在无限方向上，使得从该节点通过相应界面节点的直线在界面另一侧距离无限单元等于这些节点之间距离的位置穿过该射线的极点（[图3.3.1-1](03s03a68.md)）。
