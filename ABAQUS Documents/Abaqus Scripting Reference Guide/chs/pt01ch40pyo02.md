# 40.2 DGCommonOptions 对象

DGCommonOptions 对象存储所有绘图状态共用的值和属性。DGCommonOptions 对象没有构造函数命令。创建显示组实例时，Abaqus 使用 *odbDisplay.commonOptions* 的值创建 *odbDisplayOptions.commonOptions* 成员。
...
DGCommonOptions 对象可以具有以下成员：

*renderStyle*

 SymbolicConstant，指定绘图的渲染样式。可选值为 WIREFRAME、FILLED、HIDDEN 和 SHADED。默认值为 SHADED。
