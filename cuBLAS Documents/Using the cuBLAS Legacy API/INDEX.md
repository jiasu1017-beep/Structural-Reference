# Using the cuBLAS Legacy API

## 使用 cuBLAS Legacy API

**来源 / Source:** NVIDIA cuBLAS Library Documentation - Using the cuBLAS Legacy API

---

## 双语目录对照 / Bilingual Table of Contents

| 章节号 | 中文 Chinese | 英文 English |
|--------|--------------|--------------|
| 6 | [使用 cuBLAS Legacy API](./chs/using-the-cublas-legacy-api.md) | [Using the cuBLAS Legacy API](./eng/using-the-cublas-legacy-api.md) |
| 6.1 | [错误状态](./chs/using-the-cublas-legacy-api.md) | [Error Status](./eng/using-the-cublas-legacy-api.md) |
| 6.2 | [初始化和关闭](./chs/using-the-cublas-legacy-api.md) | [Initialization and Shutdown](./eng/using-the-cublas-legacy-api.md) |
| 6.3 | [线程安全](./chs/using-the-cublas-legacy-api.md) | [Thread Safety](./eng/using-the-cublas-legacy-api.md) |
| 6.4 | [内存管理](./chs/using-the-cublas-legacy-api.md) | [Memory Management](./eng/using-the-cublas-legacy-api.md) |
| 6.5 | [标量参数](./chs/using-the-cublas-legacy-api.md) | [Scalar Parameters](./eng/using-the-cublas-legacy-api.md) |
| 6.6 | [辅助函数](./chs/using-the-cublas-legacy-api.md) | [Helper Functions](./eng/using-the-cublas-legacy-api.md) |
| 6.7 | [Level-1、2、3 函数](./chs/using-the-cublas-legacy-api.md) | [Level-1,2,3 Functions](./eng/using-the-cublas-legacy-api.md) |
| 6.8 | [从旧版迁移到 cuBLAS API](./chs/converting-legacy-to-the-cublas-api.md) | [Converting Legacy to the cuBLAS API](./eng/converting-legacy-to-the-cublas-api.md) |
| 6.9 | [示例](./chs/using-the-cublas-legacy-api.md) | [Examples](./eng/using-the-cublas-legacy-api.md) |

---

## 翻译状态 / Translation Status

| 状态 Status | 数量 Count |
|-------------|------------|
| ✅ 已完成 / Completed | 10 |
| ⏳ 未转换 / Not converted | 0 |
| **总计 Total** | **10** |

---

## 版权声明 / Copyright Notice

本翻译仅供学习研究使用，原文档版权归 NVIDIA Corporation 所有。

This translation is for study and research purposes only. Original documentation copyright NVIDIA Corporation.
