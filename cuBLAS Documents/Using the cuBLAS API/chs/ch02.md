...
See See [cublasSetFixedPointEmulationMantissaControl()](#cublassetfixedpointemulationmantissacontrol)  and [cublasGetFixedPointEmulationMaxMantissaBitCount()](#cublasgetfixedpointemulationmaxmantissabitcount).
...
floating point emulation algorithms. See [cublasSetEmulationSpecialValuesSupport()](#cublassetemulationspecialvaluessupport) and [cublasGetEmulationSpecialValuesSupport()](#cublasgetemulationspecialvaluessupport).
...
This function sets the cuBLAS library stream, which will be used to execute all subsequent calls to the cuBLAS library functions. If the cuBLAS library stream is not set, all kernels use the *default* NULL stream. In particular, this routine can be used to change the stream between kernel launches and then to reset the cuBLAS library stream back to NULL. Additionally this function unconditionally resets the cuBLAS library workspace back to the default workspace pool (see [cublasSetWorkspace()](#cublassetworkspace)).
...
This function sets the pointer mode used by the cuBLAS library. The *default* is for the values to be passed by reference on the host. Please see the section on the [cublasPointerMode_t](#cublaspointermode-t) type for more details.
