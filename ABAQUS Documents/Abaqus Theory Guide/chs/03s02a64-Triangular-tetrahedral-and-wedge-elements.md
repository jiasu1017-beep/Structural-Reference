...
插值用[图3.2.6-1](03s02a64.md)所示的单元坐标*g*、*h*和*r*来定义。因为Abaus是大多数应用的Lagrangian代码，这些也是材料坐标。它们每个在单元中从0跨域到1，但满足约束：对于三角形和楔形为![](../graphics/stm_eqn03027.gif)，对于四面体为![](../graphics/stm_eqn03028.gif)。Abaqus用于这些单元的节点编号约定也在[图3.2.6-1](03s02a64.md)中显示。角节点首先编号，然后是二阶单元的边中节点。插值函数如下。
