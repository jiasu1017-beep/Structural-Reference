...
此问题用于验证单个梁单元在小位移响应和大旋转响应中均表现出稳定的行为。在第一种情况下，梁在轴向、弯曲、剪切和扭转（三维梁 only）变形模式下加载，并允许自由振动。第二种情况测试梁绕其端点之一的刚体旋转。在两种情况下，二维和三维梁都在有和没有体积粘度的情况下进行测试。二维和三维管道单元也针对类似于管道截面梁单元的变形进行测试。

### 变形测试

这些测试包括三个步骤。在第一步中，将梁的体积粘度设置为零，并使用平滑步幅值将位移或旋转施加到梁的端部。在第二步中，移除位移约束，并允许梁自由振荡。最后，在第三步中，将体积粘度设置为 0.06 的值，并允许梁在阻尼下振荡。所有步骤都使用固定时间增量。此时间增量策略使用基于步骤开始时单元稳定时间增量估计的临界值的时间增量。它用于避免在使用默认时间增量策略而没有体积粘度时可能发生的解中噪声的传播。通常，默认体积粘度会阻尼并防止这种高频噪声的传播。

### 刚体旋转测试

这些测试包括两个步骤。将初始速度施加到梁以引起旋转，并施加初始轴向应力以模拟旋转体中产生的离心应力。在第一步中，将体积粘度设置为零，并允许梁绕其端点旋转 5 整圈。在第二步中，将体积粘度设置为 0.06，并允许梁再旋转 5 圈。在二维情况下，旋转轴是 z 轴。在三维情况下，旋转轴位于 X–Y 平面中，与原始 y 轴成 45° 角。
...
此问题表明，Abaqus/Explicit 中使用的梁单元为自由振动和阻尼振动提供稳定的行为。[图 1.3.27--1](ch01s03abv30.md#exxbeams-xsect-axial)、[图 1.3.27--2](ch01s03abv30.md#exxbeams-xsect-bend) 和 [图 1.3.27--3](ch01s03abv30.md#exxbeams-xsect-shear) 分别显示了箱形截面二维梁在轴向、弯曲和剪切加载下的典型位移和旋转结果。所有位移和旋转都表现出小于或等于步骤 1 中施加的值。

轴向加载梁的能量平衡较差，如图 [图 1.3.27--4](ch01s03abv30.md#exxbeams-energies-fixed) 所示。这种不准确发生在因为使用太少的增量来预测梁轴向响应的每个周期。这种不准确仅发生在轴向加载情况下，因为其他模式的振动周期明显更高，因此在每个振动周期中包含更多的时间增量。位移响应和能量平衡可以通过使用直接时间积分更准确地获得。使用时间增量 1 × 10^(-4) 的直接时间积分获得的二维箱形截面梁轴向响应结果如图 [图 1.3.27--5](ch01s03abv30.md#exxbeams-xsect-axial-direct)（位移）和 [图 1.3.27--6](ch01s03abv30.md#exxbeams-energies-direct)（能量平衡）所示。

#### 旋转测试结果

对于二维和三维情况，所有轴向应变都为零。x 方向的位移在整个旋转范围内随时间正弦变化，振幅恒定。x 方向位移与时间的关系图如图 [图 1.3.27--7](ch01s03abv30.md#exxbeams-b21disp-v-time)（二维情况）和 [图 1.3.27--8](ch01s03abv30.md#exxbeams-b31disp-v-time)（三维情况）所示。
...
**图 1.3.27–1** B21 箱形截面，带轴向位移。

![](../graphics/exxbeams-xsect-axial.png)

**图 1.3.27–2** B21 箱形截面，带弯曲。

![](../graphics/exxbeams-xsect-bend.png)

**图 1.3.27–3** B21 箱形截面，带剪切位移。
...
**图 1.3.27–5** B21 箱形截面，带轴向位移（直接解时间增量控制）。

![](../graphics/exxbeams-xsect-axial-direct.png)

**图 1.3.27–6** 轴向位移的能量（直接解时间增量控制）。

![](../graphics/exxbeams-energies-direct.png)

**图 1.3.27–7** B21 x 方向位移与时间的关系。

![](../graphics/exxbeams-b21disp-v-time.png)

**图 1.3.27–8** B31 x 方向位移与时间的关系。
