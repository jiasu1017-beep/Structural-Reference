...
返回表示光标在组件中相对位置的 (status, x, y, mouseButtonState) 序列。

### getDefaultHeight()

返回此窗口的默认高度。

在 FX4Splitter、FXArrowButton、FXCheckButton、FXColorBar、FXColorWell、FXColorWheel、FXComboBox、FXComposite、FXDial、FXDockSite、FXDockTitle、FXDragCorner、FXFrame、FXGroupBox、FXHeader、FXHorizontalFrame、FXLabel、FXList、FXListBox 等中重新实现。
...
获取此窗口的 x 坐标，以父窗口的坐标系统表示。

### getY()

获取此窗口的 y 坐标，以父窗口的坐标系统表示。

### grab(confineTo=None)

将鼠标抓取到此窗口；未来的鼠标事件将报告给此窗口，即使光标在此窗口之外。
...
返回指定子窗口的索引（从零开始），如果该窗口不是子窗口或为 NULL 则返回 -1。
...
| --- | --- |
| **LAYOUT_SIDE_TOP** | 包装在顶部（默认）。 |
| **LAYOUT_SIDE_BOTTOM** | 包装在底部。 |
| **LAYOUT_SIDE_LEFT** | 包装在左侧。 |
| **LAYOUT_SIDE_RIGHT** | 包装在右侧。 |
...
| **LAYOUT_MIN_WIDTH** | 默认最小宽度。 |
| **LAYOUT_MIN_HEIGHT** | 默认最小高度。 |
