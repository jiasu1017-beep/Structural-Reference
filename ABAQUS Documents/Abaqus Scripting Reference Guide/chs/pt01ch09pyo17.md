# 9.17 DisplacementBaseMotionBC 对象

DisplacementBaseMotionBC 对象存储位移基准运动边界条件的数据。

DisplacementBaseMotionBC 对象派生自 [BoundaryCondition](pt01ch09pyo01.md) 对象。
...
此方法创建 DisplacementBaseMotionBC 对象。
...
此方法修改创建该对象的步骤中现有 DisplacementBaseMotionBC 对象的数据。
...
`setValues` 的可选参数与 [DisplacementBaseMotionBC](pt01ch09pyo17.md#ker-displacementbasemotionbc-displacementbasemotionbc-pyc) 方法的参数相同，但 *name* 和 *createStepName* 参数除外。
...
此方法修改指定步骤中现有 DisplacementBaseMotionBC 对象的传播数据。
...
DisplacementBaseMotionBC 对象可以具有以下成员：
