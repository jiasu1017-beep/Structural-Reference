# 25.63 SelfContactStdState 对象

SelfContactStdState 对象在步骤中存储 [SelfContactStd](pt01ch25pyo62.md) 对象的传播数据。SelfContactStd 对象会为每个步骤内部创建一个此对象的实例。该实例也会由 SelfContactStd 对象内部删除。

SelfContactStdState 对象没有构造函数或方法。

SelfContactStdState 对象派生自 [InteractionState](pt01ch25pyo49.md) 对象。
...
字符串，指定与此交互关联的 [ContactProperty](pt01ch25pyo21.md) 对象。
