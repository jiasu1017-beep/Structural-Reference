...
The BasicOptions object stores values and attributes associated with an [OdbDisplay](pt01ch35pyo01.md) object. The BasicOptions object has no constructor command. Abaqus creates the *defaultOdbDisplay.basicOptions* member when you import the Visualization module. Abaqus creates a *basicOptions* member when it creates the [OdbDisplay](pt01ch35pyo01.md) object, using the values from *defaultOdbDisplay.basicOptions*. Abaqus creates the *odbDisplay* member when a viewport is created, using the attributes from the previous active viewport. The previous active viewport contains the attributes from the *defaultOdbDisplay* object for the session. The attributes from the *defaultOdbDisplay* object are copied from the previous active viewport to create the new viewport.
...
A Float specifying the nodal averaging threshold percentage. Possible values are 0 ![](../graphics/ker_eqn00048.gif) *averagingThreshold* ![](../graphics/ker_eqn00048.gif) 100. The default value is 75.0.
...
A sequence of Ints specifying which sectors to display when *sectorSelectionType*=SELECT_BY_NUMBER. Possible values are 1 ![](../graphics/ker_eqn00013.gif) *selectedSectorNumbers* ![](../graphics/ker_eqn00013.gif) the number of sectors. The default value is (1).

*sweepSectorStartAngle*

A Float specifying the angle (in degrees) from which to sweep cyclic symmetry sectors when *sweepSectors*=ON. Possible values are multiples of the sector angle such that 0 ![](../graphics/ker_eqn00013.gif) *sweepSectorStartAngle* ![](../graphics/ker_eqn00013.gif) 360. The default value is 0.0.

*sweepSectorEndAngle*

A Float specifying the angle (in degrees) through which to sweep cyclic symmetry sectors when *sweepSectors*=ON. Possible values are multiples of the sector angle such that 0 ![](../graphics/ker_eqn00048.gif) *sweepSectorEndAngle* ![](../graphics/ker_eqn00013.gif) 360. The default value is 360.0.
...
If *transformationType*=NODAL, Abaqus will transform nodal vector fields into any orientation defined in the analysis with the [*TRANSFORM](../key/key-link.md#usb-kws-mtransform) option. Setting *transformationType*=NODAL has no effect on element-based results. 
...
A [DatumCsys](pt01ch15pyo03.md) object specifying the coordinate system to use for results transformation when *transformationType*=USER_SPECIFIED. 
...
A tuple of Ints specifying which sectors to display when *sectorSelectionType*=SELECT_BY_NUMBER. Possible values are 1 ![](../graphics/ker_eqn00013.gif) *selectedSectorNumbers* ![](../graphics/ker_eqn00013.gif) the number of sectors. The default value is (1).
