...
当本构积分完成时，此方程组右侧的所有项都是已知的，因此这些方程明确地定义![](../graphics/stm_eqn05697)。
...
后向Euler方法是隐式的；由于塑性应变率通常是应力的强函数，必须小心开发有效算法来求解使用此算子产生的非线性代数方程。问题已在"塑性模型的积分，"第4.2.2节中正式提出。主要困难是获得![](../graphics/stm_eqn05568)的合理起始猜测。为此我们如下进行。
...
代数问题的求解通过首先找到![](../graphics/stm_eqn05708)和![](../graphics/stm_eqn05709)的合理初始猜测，然后求解完整问题来获得。

Mises和Hill等效应力定义（![](../graphics/stm_eqn05702)都具有以下性质：
...
![](../graphics/stm_eqn05708)和![](../graphics/stm_eqn05709)的初始估计通过将问题投影到![](../graphics/stm_eqn05712)和![](../graphics/stm_eqn00064)获得，其中![](../graphics/stm_eqn05712)是![](../graphics/stm_eqn05713)，在![](../graphics/stm_eqn05714)定义，即如果在增量期间没有非弹性变形将发生在增量结束时出现的应力状态。投影为
...
[公式4.3.4-2](04s03a106.md)到[公式4.3.4-5](04s03a106.md)是一组非线性方程，可以求解![](../graphics/stm_eqn05708)和![](../graphics/stm_eqn05709)。我们用Newton方法求解这些方程，然后使用此解作为求解完整问题的起始估计。当使用Mises应力势且问题不是平面应力时，此起始估计是完整问题的解，因为Mises应力势在偏平面上是一个圆。
### 用户子程序CREEP

Abaqus/Standard为实现粘塑性模型（如蠕变和溶胀）提供了一个非常通用的能力，其中应变率势可以写为等效压力应力*p*、Mises或Hill等效偏应力![](../graphics/stm_eqn05722)的函数，以及任意数量的解相关状态变量。本节的目的是提供用户子程序CREEP中需要执行的操作的概述。为说明主要思想，蠕变律假定为应变硬化类型，形式为
...
认识到![](../graphics/stm_eqn05728)并且![](../graphics/stm_eqn05729)是![](../graphics/stm_eqn05730)的函数，隐式积分方案导致蠕变应变增量![](../graphics/stm_eqn05708中的非线性方程，Abaqus在每个材料点迭代求解（这些是局部材料点迭代，与全局平衡迭代不同）。用户子程序在每次局部迭代中的每个积分点被调用。用于迭代求解上述非线性方程的Newton-Raphson方案由
