...
The simulation of the pin-jointed, overhead hoist in [Figure 2-1](#gss-schematic-hoist) is used to illustrate the creation of an Abaqus input file using an editor. As you read through this section, you should type the data into a file using one of the editors available on your computer. The Abaqus input file must have an `.inp` file extension. For convenience, name the input file `frame.inp`. The file identifier, which can be chosen to identify the analysis, is called the *jobname*. In this case use the jobname "frame" to associate it easily with the input file called `frame.inp`.

All of the other examples in this guide assume that you will be using a preprocessor, such as Abaqus/CAE, to generate the mesh if you are going to create the model from scratch. Input files for all the examples are available. See [Appendix A, "Example Files"](../key/ap01.html) for instructions on how to retrieve these input files. However, since the purpose of this example is to help you understand the structure and format of the Abaqus input file, you should type this input file in directly, rather than use a preprocessor or copy the input file that is provided. If you wish to create the entire model using Abaqus/CAE, refer to [Getting Started with Abaqus: Interactive Edition, Section 2.3](book:../gsa/gsa-link.htm#gsa-abs-model).

## 2.3.1 Units

Before starting to define this or any model, you need to decide which system of units you will use. Abaqus has no built-in system of units. Do not include unit names or labels when entering data in Abaqus. All input data must be specified in consistent units. Some common systems of consistent units are shown in [Table 2-1](#gsa-abasics-table-units).
...
You also need to decide which coordinate system to use. The global coordinate system in Abaqus is a right-handed, rectangular (Cartesian) system. For this example define the global 1-axis to be the horizontal axis of the hoist and the global 2-axis to be the vertical axis ([Figure 2-3](#gss-model)). The global 3-axis is normal to the plane of the framework. The origin (X=0, Y=0, Z=0) is the bottom left-hand corner of the frame.

**Figure 2-3** Coordinate system and origin for model.

![Coordinate system and origin for model](../graphics/gss-model-nls.png)
...
You must select the element types and design the mesh. Creating a proper mesh for a given problem requires experience. For this example you will use a single truss element to model each member of the frame, as shown in [Figure 2-4](#gss-finite-elem).

**Figure 2-4** Finite element mesh.

![Finite element mesh](../graphics/gss-finite-elem-nls.png)

A truss element, which can carry only tensile and compressive axial loads, is ideal for modeling pin-jointed frameworks, such as the overhead hoist. Truss elements are described in ["Truss elements," Section 3.1.5](ch03s01.html#gsk-gen-elm-trusselem), and also in the [Abaqus Analysis User's Guide](../usb/usb-link.htm#usb), which describes every element available in Abaqus. The index of element types ([Section EI.1, "Abaqus/Standard Element Index," of the Abaqus Analysis User's Guide](../usb/usb-link.htm#usb-idx-stdelementindex)) makes locating a particular element easy. Whenever you are using an element for the first time, you should read the description, which includes the element connectivity and any element section properties needed to define the element's geometry.

The connectivity for the truss elements used in the overhead hoist model is shown in [Figure 2-5](#gss-truss-elem).

**Figure 2-5** Connectivity for the 2-node truss element (T2D2).

![Connectivity for the 2-node truss element (T2D2)](../graphics/gss-truss-elem.png)
...
In this case we use the node and element numbers shown in [Figure 2-6](#gss-hoist-model).

**Figure 2-6** Node and element numbers for the hoist model.

![Node and element numbers for the hoist model](../graphics/gss-hoist-model.png)
...
```
*HEADING
44	Two-dimensional overhead hoist frame
45	SI units (kg, m, s, N)
46	1-axis horizontal, 2-axis vertical
```
...
The coordinates of each node can be defined once you select the mesh design and node numbering scheme. For this problem use the numbering shown in [Figure 2-6](#gss-hoist-model). The coordinates of nodes are defined using the `*NODE` option. Each data line of this option block has the form:
...
where *node 1* and *node 2* are at the ends of the element (see [Figure 2-5](#gss-truss-elem)). For example, element 16 connects nodes 103 and 105 (see [Figure 2-6](#gss-hoist-model)), so the data line defining this element is:
...
The complete `*ELEMENT` option block for the overhead hoist model (see [Figure 2-6](#gss-hoist-model)) is shown below:
...
Each element must refer to an element section property. The appropriate element section option for each element and the additional geometric data (if any) needed for each element are described in the [Abaqus Analysis User's Guide](../usb/usb-link.htm#usb).
...
In this case all the members are circular bars that are 5 mm in diameter. Their cross-sectional area is 1.963 × 10^-5^ m^2^.
...
![Element section input](../graphics/gss-cross-sect-input-nls.png)
...
Abaqus has a large number of material models, many of which include nonlinear behavior. In this overhead hoist example we use the simplest form of material behavior: linear elasticity. In [Chapter 10, "Materials"](ch10.html), two of the most common forms of nonlinear material behavior are considered: metal plasticity and rubber elasticity. A discussion of all the material models available in Abaqus can be found in the [Abaqus Analysis User's Guide](../usb/usb-link.htm#usb).

Linear elasticity is appropriate for many materials at small strains, particularly for metals up to their yield point. It is characterized by a linear relationship between stress and strain (Hooke's law), as shown in [Figure 2-7](#gss-linear-elast).

**Figure 2-7** Linear elastic material.

![Linear elastic material](../graphics/gss-linear-elast-nls.png)
...
In this example we are interested in the static response of the overhead hoist to a 10 kN load applied at the midspan, with the left-hand end fully constrained and a roller constraint on the right-hand end (see [Figure 2-1](#gss-schematic-hoist)). This is a single event, so only a single step is needed for the simulation.
...
```
*STEP, PERTURBATION
65	10kN central load
```

The `PERTURBATION` parameter indicates that this is a linear analysis. If this parameter is omitted, the analysis may be linear or nonlinear. The use of the `PERTURBATION` parameter is discussed further in [Chapter 11, "Multiple Step Analysis"](ch11.html).
...
| XSYMM | Symmetry constraint about a plane of constant X. |
| YSYMM | Symmetry constraint about a plane of constant Y. |
| ZSYMM | Symmetry constraint about a plane of constant Z. |
| XASYMM | Antisymmetry constraint about a plane of constant X. |
| YASYMM | Antisymmetry constraint about a plane of constant Y. |
| ZASYMM | Antisymmetry constraint about a plane of constant Z. |
...
In this example all of the constraints are in the global 1- or 2-directions. In many cases constraints are required in directions that are not aligned with the global directions. The `*TRANSFORM` option can be used in such cases to define a local coordinate system for boundary condition application. The skew plate example in [Chapter 5, "Using Shell Elements"](ch05.html), demonstrates how to use this option in such cases.
...
The input file is now complete. Compare the input file you have generated to the complete input file given in [Figure 2-2](#gss-hoist-input). Save the data as `frame.inp`, and exit the editor.
...
The history data are presented below in two sections. The first line of the top half of the history data reads 10kN central load, which is the first data line given in the `*STEP` option block. This line reminds you of the loads applied in this step.
...
1. From the main menu bar, select **File** → **Open**; or use the ![File Open tool](../graphics/ico_fileOpen.png) tool in the **File** toolbar.
...
3. Click **OK**.
   ...
   **Tip:** You can also open the output database `frame.odb` by typing the following command at the operating system prompt:
   ...
   Abaqus/Viewer opens the output database created by the job and displays the undeformed model shape, as shown in [Figure 2-8](#gsa-undeformed).

**Figure 2-8** Undeformed model shape.

![Undeformed model shape](../graphics/gss-undeformed-v.png)

You can choose to display the title block and state block at the bottom of the viewport; these blocks are not shown in [Figure 2-8](#gsa-undeformed). The title block at the bottom of the viewport indicates the following:
...
1. From the main menu bar, select **Options** → **Common**; or use the ![Common Plot Options tool](../graphics/ico_plotCommonOptions.png) tool in the toolbox.
...
The customized undeformed plot is shown in [Figure 2-9](#gsk-node-plot).

**Figure 2-9** Node number plot.

![Node number plot](../graphics/gss-node-plot-v.png)
...
The resulting plot is shown in [Figure 2-10](#gsk-node-elem).

**Figure 2-10** Node and element number plot.

![Node and element number plot](../graphics/gss-node-elem-v.png)
...
From the main menu bar, select **Plot** → **Deformed Shape**; or use the ![Deformed Shape tool](../graphics/ico_plotDeformed.png) tool in the toolbox. Abaqus/Viewer displays the deformed model shape, as shown in [Figure 2-11](#gsa-geometry).
...
1. Click the **Allow Multiple Plot States** ![Allow Multiple Plot States tool](../graphics/ico_plotMultipleStates.png) tool in the toolbox to allow multiple plot states in the viewport; then click the ![Undeformed Shape tool](../graphics/ico_plotUndeformed.png) tool or select **Plot** → **Undeformed Shape** to add the undeformed shape plot to the existing deformed plot in the viewport.
...
2. The plot options for the superimposed image are controlled separately from those of the primary image. From the main menu bar, select **Options** → **Superimpose**; or use the ![Superimpose Plot Options tool](../graphics/ico_plotSuperimposeOptions.png) tool in the toolbox to change the edge style of the superimposed (i.e., undeformed) image.
...
The plot is shown in [Figure 2-12](#gss-shapes). The undeformed model shape appears with a dashed edge style.
...
1. Click the ![Allow Multiple Plot States tool](../graphics/ico_plotMultipleStates.png) tool in the toolbox to disable multiple plot states in the viewport.
...
6. Click **OK**.
   ...
   Abaqus/Viewer displays symbols to indicate the applied boundary conditions, as shown in [Figure 2-13](#gsa-overhead-hoist).
...
2. From the main menu bar, select **Options** → **Animation**; or use the animation options ![Animation Options tool](../graphics/ico_selectionOptions.png) tool in the toolbox (located directly underneath the ![Time History Animation tool](../graphics/ico_animTimeHistory.png) tool).
...
4. You can use the animation controls to start, pause, and step through the animation. From left to right of [Figure 2-14](#usv-anm-moviecontrols-nls), these controls perform the following functions: **play/pause**, **first**, **previous**, **next**, and **last**.
...
2. From the list of available history output, double-click `Spatial displacement: U2 at Node 102 in NSET CENTER`.
   ...
   Abaqus/Viewer plots the vertical displacement at the center node along the bottom of the truss, as shown in [Figure 2-15](#gsa-hoist-displacement).
   ...
   **Note:** The chart legend has been suppressed and the axis labels modified in this figure. Many *X-Y* plot options are directly accessible by double-clicking the appropriate regions of the viewport. To enable direct object actions, however, you must first click ![Cancel](../graphics/afxI_cancel.png) in the prompt area to cancel the current procedure (if necessary). To suppress the legend, double-click it in the viewport to open the **Chart Legend Options** dialog box. In the **Contents** tabbed page of this dialog box, toggle off **Show legend**. To modify the axis labels, double-click either axis to open the **Axis Options** dialog box, and edit the axis titles as indicated in [Figure 2-15](#gsa-hoist-displacement).
