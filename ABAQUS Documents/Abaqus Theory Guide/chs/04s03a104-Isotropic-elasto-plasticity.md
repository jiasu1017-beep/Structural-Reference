...
弹性是线性各向同性的，因此，可以用两个温度相关材料参数写为。出于此推导的目的，最合适的是选择这些参数为体积模量*K*和剪切模量*G*。这些可以从用户输入的杨氏模量*E*和泊松比![](../graphics/stm_eqn01854)计算为
...
此方程可以求逆（必要时数值地）给出*q*作为增量结束时![](../graphics/stm_eqn05621)的函数。
...
[公式4.3.2-1](04s03a104.md)到[公式4.3.2-7](04s03a104.md)定义材料行为。在任何发生塑性流动的增量中（由基于纯弹性响应评估*q*并发现其值超过![](../graphics/stm_eqn05285)来确定），这些方程必须被积分并求解增量结束时的状态。如"金属塑性模型，"第4.3.1节中的一般讨论，积分通过将后向Euler方法应用于流动规则（[公式4.3.2-4](04s03a104.md)）完成，给出
...
使用积分流动规则（[公式4.3.2-8](04s03a104.md)），连同Mises流动方向定义，![](../graphics/stm_eqn00483)（在[公式4.3.2-4](04s03a104.md)中），这变为
...
这是一个关于![](../graphics/stm_eqn05638)的非线性方程，当![](../graphics/stm_eqn05629)依赖于等效塑性应变时（即，当材料是率相关的，或当存在非零加工硬化时）。（对于率无关完美塑性，它是关于![](../graphics/stm_eqn05639的线性。）我们用Newton方法求解：

![](../graphics/stm_eqn05640.gif)

![](../graphics/stm_eqn05641.gif)我们迭代直到收敛。

一旦![](../graphics/stm_eqn05642)已知，解被完全定义：使用[公式4.3.2-5](04s03a104.md)，
...
对于平面应力，![](../graphics/stm_eqn02690)不由运动学定义，而是由平面应力约束

![](../graphics/stm_eqn05648.gif)

这个附加方程（或等效地![](../graphics/stm_eqn05649)）必须与屈服条件和[公式4.3.2-9](04s03a104.md)一起求解。预测的第三应变分量
...
从![](../graphics/stm_eqn05661)的定义（见[公式4.3.2-11](04s03a104.md)），
