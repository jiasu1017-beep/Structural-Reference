# 9.9 ConcentrationBC 对象

ConcentrationBC 对象存储浓度边界条件的数据。

ConcentrationBC 对象派生自 [BoundaryCondition](pt01ch09pyo01.md) 对象。
...
### 9.9.1 ConcentrationBC(...)

此方法创建一个 ConcentrationBC 对象。
...
一个 Float，指定浓度幅值。默认值为 0。如果 *distributionType*=USER_DEFINED，则 *magnitude* 参数是可选的。
...
此方法修改现有 ConcentrationBC 对象在创建它的步骤中的数据。
...
`setValues` 的可选参数与 [ConcentrationBC](pt01ch09pyo09.md#ker-concentrationbc-concentrationbc-pyc) 方法的参数相同，但 *name* 和 *createStepName* 参数除外。
...
### 9.9.3 setValuesInStep(...)

此方法修改指定步骤中现有 ConcentrationBC 对象的传播数据。
...
一个 Float 或 SymbolicConstant FREED，指定浓度幅值。
