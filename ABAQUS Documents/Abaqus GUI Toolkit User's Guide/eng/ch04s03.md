...
In this example you will use three-dimensional, continuum elements to model the connecting lug shown in [Figure 4--14](ch04s03.md#gss-connecting-lug).
...
- Rather than include the complex bolt-lug interaction in the model, you will use a distributed pressure over the bottom half of the hole to load the connecting lug (see [Figure 4--14](ch04s03.md#gss-connecting-lug)).
...
An Abaqus input file always starts with the [*HEADING](../key/key-link.md#usb-kws-mheading) option. Often the description given in this option by the preprocessor is not very informative, although it might give the date and time when the file was generated. You should provide a suitable title on the data lines of this option so that someone looking at this file can tell what is being modeled and what units you used. The [*HEADING](../key/key-link.md#usb-kws-mheading) option block used in `lug.inp` appears below:
...
In input files created by a preprocessor, the model's nodal coordinates usually are in one large [*NODE](../key/key-link.md#usb-kws-mnode) option block, with the coordinates specified for each node individually.

The element definitions generated by the preprocessor usually are contained in several [*ELEMENT](../key/key-link.md#usb-kws-melement) option blocks. Typically, each block contains elements that have the same element section and material properties. In the connecting lug model only one element type is used, and all the elements have the same properties. Therefore, there will probably be a single [*ELEMENT](../key/key-link.md#usb-kws-melement) option block in your input file. It will look similar to
...
Here three data lines are used to define the connectivity of one C3D20R element completely (a minimum of two is required). If a data line in an [*ELEMENT](../key/key-link.md#usb-kws-melement) option block ends with a comma, it indicates that the next data line contains more nodes defining the current element. The parameter `ELSET=LUG` indicates that all the elements defined in the following data lines will be stored in an element set called `LUG`. If your model does not have a descriptive element set name in the [*ELEMENT](../key/key-link.md#usb-kws-melement) option, change it to `LUG`.
...
You define sets using the [*NSET](../key/key-link.md#usb-kws-mnset) and [*ELSET](../key/key-link.md#usb-kws-melset) options in the input file. The name of a set is specified with either the NSET or ELSET parameter. The data lines list the nodes or elements that are included in the set. Each data line can contain up to 16 numbers, and there can be as many data lines as required. For example, the node set `LHEND` (see [Figure 4--16](ch04s03.md#gss-elementsets)) can be defined as
...
The connecting lug is made of a mild steel and, thus, has isotropic, linear elastic material behavior under the loads being applied. Assume that *E* = 200 GPa and that ![](../graphics/gsk_eqn00028.gif) = 0.3. These are given on the data line of the [*ELASTIC](../key/key-link.md#usb-kws-melastic) option (remember the overhead hoist example in [Chapter 2, "Abaqus Basics](ch02.md)”). The following material property definition specifies these properties in the input file:
...
The value for the NAME parameter on the [*MATERIAL](../key/key-link.md#usb-kws-mmaterial) option must match the value of the MATERIAL parameter on the [*SOLID SECTION](../key/key-link.md#usb-kws-msolidsection) option.

### 4.3.5 Reviewing the input file---the history data

The history data portion of the input file starts at the first [*STEP](../key/key-link.md#usb-kws-hstep) option. Many preprocessors create a linear static step in the input file by default. This example will use a general static step. The following options define the step:
...
```
 ...
 If these options are not in your input file, add them at the end of the existing data. It is easier for someone else to understand your model if you use the data lines following the [*STEP](../key/key-link.md#usb-kws-hstep) option to add a suitable title describing the event being simulated in the step.

**Boundary conditions**

In the model of the connecting lug, all the nodes need to be constrained in all three directions at the left-hand end where it is attached to its parent structure (see [Figure 4--19](ch04s03.md#gss-lug)). 

**Figure 4–19** Boundary conditions on the connecting lug.

![](../graphics/gss-lug.png)

In this example, each constrained degree of freedom is specified individually in the [*BOUNDARY](../key/key-link.md#usb-kws-hboundary) option block, as shown below:
...
If you think that you defined the boundary conditions incorrectly, you can display them in Abaqus/Viewer and compare them with the boundary conditions shown in [Figure 4--19](ch04s03.md#gss-lug). The postprocessing instructions given at the end of ["Postprocessing," Section 2.3.9](ch02s03.md#gsk-gen-abs-postprocessing), discuss how to do this.

**Loading**

The lug carries a pressure of 50 MPa distributed around the bottom-half of the hole. Pressure loads can be defined conveniently using the preprocessor by selecting the element faces to which the load is applied. In the connecting lug input file, these loads will appear as a [*DLOAD](../key/key-link.md#usb-kws-hdload) option block. For example, the [*DLOAD](../key/key-link.md#usb-kws-hdload) option block for the connecting lug may look like
...
You can define several [*NODE PRINT](../key/key-link.md#usb-kws-hnodeprint) and [*EL PRINT](../key/key-link.md#usb-kws-helprint) options. The parameter TOTALS=YES causes the sum of the reaction forces at all the nodes in the node set to be printed. The SUMMARY=NO parameter prevents the minimum and maximum values in the table from being printed.
...
You will use the NFORC output variable to create and display free body cuts in ["Postprocessing---visualizing the results," Section 4.3.8](ch04s03.md#gsk-gen-ctm-postprocessing). The following options write the nodal forces due to the element stresses to the output database while also writing the default output:
...
If you modified any input data, store the input in a file called `lug.inp` (an example file is listed in ["Connecting lug," Section A.2](ap01s02.md)). Then, run the simulation using the command:

```
abaqus job=lug interactive
```

When the job has completed, check the data file, `lug.dat`, for any errors or warnings. If there are any errors, correct the input file and run the simulation again. If you have problems correcting any errors, try comparing your input file to the one given in ["Connecting lug," Section A.2](ap01s02.md). Check that you have the correct parameters for each input option.

### 4.3.7 Results

When the job has completed successfully, look at the three tables of output that you requested. They will be found at the end of the data file. A portion of the table of element stresses is shown in [Figure 4--21](ch04s03.md#gss-c4-int-results-inp). The maximum Mises stress at the built-in end is approximately 306 MPa.

**Figure 4–21** Table of element stresses in the data file.

![](../graphics/gss-c4-int-results-inp-nls.png)

The tables showing the displacements of the nodes along the bottom of the hole and the reaction forces at the constrained nodes are shown in [Figure 4--22](ch04s03.md#gss-c4-node-results-inp) and [Figure 4--23](ch04s03.md#gss-c4-tot-react-inp), respectively. 
...
From the main menu bar, select ****Plot**![](../graphics/images/arrow.gif)**Deformed Shape****; or use the ![](../graphics/ico_plotDeformed.png) tool in the toolbox. [Figure 4--24](ch04s03.md#gsa-displacedshape-c) displays the deformed model shape at the end of the analysis. What is the displacement magnification level?
...
1. From the main menu bar, select ****View**![](../graphics/images/arrow.gif)**Specify**** (or double-click the 3D compass). The **Specify View** dialog box appears.
2. From the list of available methods, select **Viewpoint**. In the **Viewpoint** method, you enter three values representing the *X*-, *Y*-, and *Z*-position of an observer. You can also specify an up vector. Abaqus positions your model so that this vector points upward.
3. Enter the *X*-, *Y*-, and *Z*-coordinates of the viewpoint vector as `1, 1, 3` and the coordinates of the up vector as `0, 1, 0`.
4. Click **OK**. Abaqus/Viewer displays your model in the specified view, as shown in [Figure 4--25](ch04s03.md#gss-shaded-plot-c). **Figure 4--25** Deformed model shape viewed from specified viewpoint. ![](../graphics/gss-shaded-plot-v.png)

**Visible edges**

Several options are available for choosing which edges will be visible in the model display. The previous plots show all exterior edges of the model; [Figure 4--26](ch04s03.md#gss-wireframe-c) displays only feature edges.
...
1. From the main menu bar, select ****Options**![](../graphics/images/arrow.gif)**Common****. The **Common Plot Options** dialog box appears.
2. Click the **Basic** tab if it is not already selected.
3. From the **Visible Edges** options, choose **Feature edges**.
4. Click **Apply**. The deformed plot in the current viewport changes to display only feature edges, as shown in [Figure 4--26](ch04s03.md#gss-wireframe-c).

**Render style**

A shaded plot is a filled plot in which a lightsource appears to be directed at the model. This is the default render style and can be very useful when viewing complex three-dimensional models. Three other render styles provide additional display options: wireframe, hidden line, and filled. You can select a render style from the **Common Plot Options** dialog box or from the tools on the **Render Style** toolbar: wireframe ![](../graphics/ico_displayWire.png), hidden line ![](../graphics/ico_displayHidden.png), filled ![](../graphics/ico_displayFilled.png), and shaded ![](../graphics/ico_displayShaded.png). To display the wireframe plot shown in [Figure 4--27](ch04s03.md#gss-wireframe-all-c), select **Exterior edges** in the **Common Plot Options** dialog box, click **OK** to close the dialog box, and select wireframe plotting by clicking the ![](../graphics/ico_displayWire.png) tool. All subsequent plots will be displayed in the wireframe render style until you select another render style. 

**Figure 4–27** Wireframe plot.

![](../graphics/gss-lineplot-v.png)

A wireframe model showing internal edges can be visually confusing for complex three-dimensional models. You can use the other render style tools to select the hidden line and filled render styles, shown in [Figure 4--28](ch04s03.md#gss-lineplot-c) and [Figure 4--29](ch04s03.md#gss-elem-plot-c), respectively. These render styles are more useful when viewing complex three-dimensional models.
...
1. From the main menu bar, select ****Plot**![](../graphics/images/arrow.gif)**Contours**![](../graphics/images/arrow.gif)**On Deformed Shape****. The filled contour plot shown in [Figure 4--30](ch04s03.md#gss-mises-filled-v) appears. The Mises stress, `S Mises`, indicated in the legend title is the default variable chosen by Abaqus for this analysis. You can select a different variable to plot. **Figure 4--30** Filled contour plot of Mises stress. ![](../graphics/gss-mises-filled-v.png)
2. From the main menu bar, select ****Result**![](../graphics/images/arrow.gif)**Field Output****. The **Field Output** dialog box appears; by default, the **Primary Variable** tab is selected.
3. From the list of available output variables, select a new variable to plot.
4. Click **OK**. The contour plot in the current viewport changes to reflect your selection.

**Tip:**You can also use the **Field Output** toolbar, located above the viewport, to change the displayed field output variable. For more information, see ["Using the field output toolbar," Section 42.5.2 of the Abaqus/CAE User's Guide](../usi/usi-link.md#usv-res-foutputtoolbar).

Abaqus/Viewer offers many options to customize contour plots. To see the available options, click the **Contour Options** ![](../graphics/ico_plotContourOptions.png) tool in the toolbox. By default, Abaqus/Viewer automatically computes the minimum and maximum values shown in your contour plots and evenly divides the range between these values into 12 intervals. You can control the minimum and maximum values Abaqus/Viewer displays (for example, to examine variations within a fixed set of bounds), as well as the number of intervals. 
...
1. From the main menu bar, select ****Tools**![](../graphics/images/arrow.gif)**View Cut**![](../graphics/images/arrow.gif)**Create****.
...
3. Click **OK** to close the dialog box and to make the view cut. The view appears as shown in [Figure 4--32](ch04s03.md#gsa-cutting-plane). **Figure 4--32** Mises stress through the lug thickness. ![](../graphics/gsa-cutting-plane-v.png) From the main menu bar, select ****Tools**![](../graphics/images/arrow.gif)**View Cut**![](../graphics/images/arrow.gif)**Manager**** to open the **View Cut Manager**. By default, the regions on and below the cut are displayed (as indicated by the check marks beneath the **on cut** ![](../graphics/visG_CutOn.png) and **below cut** ![](../graphics/visG_CutBelow.png) symbols). To translate or rotate the cut, choose **Translate** or **Rotate** from the list of available motions and enter a value or drag the slider at the bottom of the **View Cut Manager**.
4. To view the full model again, toggle off **Cut-4** in the **View Cut Manager**. For more information on view cuts, see [Chapter 80, "Cutting through a model," of the Abaqus/CAE User's Guide](../usi/usi-link.md#uss-cut).
...
1. From the main menu bar, select ****Viewport**![](../graphics/images/arrow.gif)**Viewport Annotation Options****; then click the **Legend** tab in the dialog box that appears. The **Legend** options become available.
2. Toggle on **Show min/max values**.
3. Click **OK**. The contour legend changes to report the minimum and maximum contour values.
...
1. From the list of variable types on the left side of the **Field Output** toolbar, select **Primary** if it is not already selected. **Tip:**You can click ![](../graphics/ico_fieldOutput.png) on the left side of the **Field Output** toolbar to make your selections from the **Field Output** dialog box instead of the toolbar. If you use the dialog box, you must click **Apply** or **OK** for Abaqus/Viewer to display your selections in the viewport.
...
1. In the Results Tree, double-click ** Display Groups**. The **Create Display Group** dialog box opens.
2. From the **Item** list, select **Elements**. From the **Method** list, select **Internal sets**. Once you have selected these items, the list on the right-hand side of the **Create Display Group** dialog box shows the available selections.
...
4. When the highlighted set corresponds to the group of elements at the bottom of the hole, click **Replace** ![](../graphics/ico_displayGroupReplace.png) to replace the current model display with this element set. Abaqus/Viewer displays the specified subset of your model.
...
1. From the main menu bar, select ****Options**![](../graphics/images/arrow.gif)**Common****. The **Common Plot Options** dialog box appears.
2. Set the render style to filled; all visible element edges will be displayed for convenience. 1. Toggle on **Filled** under **Render Style**. 2. Toggle on **All edges** under **Visible Edges**.
3. Click the **Labels** tab, and toggle on **Show element labels** and **Show face labels**.
4. Click **Apply** to apply the plot options.
5. From the main menu bar, select ****Plot**![](../graphics/images/arrow.gif)**Undeformed Shape****; or use the ![](../graphics/ico_plotUndeformed.png) tool in the toolbox. Abaqus/Viewer displays the element and face identification labels in the current display group.
...
1. To display the entire model in the viewport, select ****Tools**![](../graphics/images/arrow.gif)**Display Group**![](../graphics/images/arrow.gif)**Plot**![](../graphics/images/arrow.gif)**All**** from the main menu bar.
2. From the main menu bar, select ****Tools**![](../graphics/images/arrow.gif)**Free Body Cut**![](../graphics/images/arrow.gif)**Manager****.
...
7. Select the surface, highlighted in [Figure 4--33](ch04s03.md#gss-fbd-face), to define the free body cut cross-section. 1. From the **Selection** toolbar, toggle off the **Select the Entity Closest to the Screen** tool ![](../graphics/ico_filterVisible.png) and ensure that the **Select From All Entities** tool ![](../graphics/ico_filterBothIntExt.png) is selected. 2. As you move the cursor in the viewport, Abaqus/CAE highlights all of the potential selections and adds ellipsis marks (...) next to the cursor arrow to indicate an ambiguous selection. Position the cursor so that one of the faces of the desired surface is highlighted, and click to display the first surface selection. **Figure 4--33** Selected faces for the free body cross-section. ![](../graphics/gss-fbd-face-gsk.png) 3. Use the **Next** and **Previous** buttons to cycle through the possible selections until the appropriate vertical surface is highlighted, and click **OK**.
...
11. From the **Free Body Plot Options** dialog box, select the **Force** tab in the **Color & Style** tabbed page. Click the resultant color sample ![](../graphics/usi-colorsample.png) to change the color of the resultant force arrow.
12. Once you have selected a new color for the resultant force arrow, click **OK** in the **Free Body Plot Options** dialog box and click **Dismiss** in the **Free Body Cut Manager**. The free body cut is displayed in the viewport, as shown in [Figure 4--34](ch04s03.md#gss-fbd-result). **Figure 4--34** Free body cut displayed on the connecting lug. ![](../graphics/gss-fbd-result-gsk.png)
...
2. In the prompt area, set the selection method to **individually**; and select the nodes at the bottom of the hole in the lug, as indicated in [Figure 4--35](ch04s03.md#gss-lug-bottomnodes-c). Click **Done** when all the nodes on the bottom of the hole are highlighted in the viewport. In the **Create Display Group** dialog box, click **Save Selection As**. Save the display group as `nodes at hole bottom`. **Figure 4--35** Nodes in display group `nodes at hole bottom`. ![](../graphics/gss-lug-bottomnodes-v-nls.png)
...
2. From the main menu bar, select ****Report**![](../graphics/images/arrow.gif)**Field Output****.
...
Second-order hexahedral elements are not available in the Abaqus/Explicit element library. Thus, you will need to change the element type specified on the [*ELEMENT](../key/key-link.md#usb-kws-melement) option from C3D20R to C3D8R. This change also requires that you edit the element nodal connectivity so that only eight nodes are specified for each element. For example, the following [*ELEMENT](../key/key-link.md#usb-kws-melement) option block was used to define one of the elements in `lug.inp`:
...
Because the C3D8R element employs reduced integration, use the enhanced strain algorithm to control hourglassing. You can specify enhanced hourglassing with the [*SECTION CONTROLS](../key/key-link.md#usb-kws-msectioncontrols) option:
...
You can modify the material definition by adding the [*DENSITY](../key/key-link.md#usb-kws-mdensity) option to the material option block. The complete material definition for the connecting lug is:
...
Revise the step definition to examine the dynamic response of the lug over a period of 0.005 s. This change requires that you change the [*STEP](../key/key-link.md#usb-kws-hstep) option block, which appeared as follows for the static analysis:
...
Write field output at 125 equally spaced intervals and also write the default history output. You can specify output at evenly spaced intervals by appending the NUMBER INTERVAL parameter to the [*OUTPUT](../key/key-link.md#usb-kws-houtput) option block. Replace the existing output requests with the following:
...
From the main menu bar, select ****Plot**![](../graphics/images/arrow.gif)**Deformed Shape****; or use the ![](../graphics/ico_plotDeformed.png) tool in the toolbox. [Figure 4--36](ch04s03.md#gsa-displaced-exp) displays the deformed model shape at the end of the analysis. 
...
2. The list of all the variables in the history portion of the output database appears; these are the only history output variables you can plot. From the list of available output variables, double-click **ALLIE** to plot the internal energy for the whole model. Abaqus reads the data for the curve from the output database file and plots the graph shown in [Figure 4--37](ch04s03.md#gsa-lugexp-allie). **Figure 4--37** Internal energy for the whole model. ![](../graphics/gsk-lugexp-allie.png)
3. Repeat this procedure to plot **ALLKE**, the kinetic energy for the whole model (shown in [Figure 4--38](ch04s03.md#gsa-lugexp-allke)). **Figure 4--38** Kinetic energy for the whole model. ![](../graphics/gsk-lug-allke-nls.png) Both the internal energy and the kinetic energy show oscillations that reflect the vibrations of the lug. Throughout the simulation, kinetic energy is transformed into internal (strain) energy and vice-versa. Since the material is linear elastic, total energy is conserved. This can be seen by plotting `ETOTAL`, the total energy of the system, together with `ALLIE` and `ALLKE`. The value of `ETOTAL` is approximately zero throughout the course of the analysis. Energy balances in dynamic analysis are discussed further in [Chapter 9, "Nonlinear Explicit Dynamics](ch09.md)."
...
6. Click **Edit Selection**. In the viewport, select one of the nodes on the bottom of the hole as shown in [Figure 4--39](ch04s03.md#gsa-lugexp-node) (if necessary, change the render style to facilitate your selection). Click **Done** in the prompt area. **Figure 4--39** Selected node at the bottom of the hole. ![](../graphics/gsk-lugexp-node-nls.png)
7. Click **Plot** in the **XY Data from ODB Field Output** dialog box to plot the nodal displacement as a function of time. The history of the oscillation, as shown in [Figure 4--40](ch04s03.md#gsa-lugexp-nodaldisp), indicates that the displacements are small (relative to the structure's dimensions). **Figure 4--40** Displacement of a node at the bottom of the hole. ![](../graphics/gsk-lugexp-nodaldisp.png) Thus, this problem could have been solved adequately using small-deformation theory. This would have reduced the computational cost of the simulation without significantly affecting the results. Nonlinear geometric effects are discussed further in [Chapter 8, "Nonlinearity](ch08.md)."
...
6. Click **Edit Selection**. In the viewport, select one of the elements near the built-in end of the lug as shown in [Figure 4--41](ch04s03.md#gsa-lugexp-elem). Click **Done** in the prompt area. **Figure 4--41** Selected element near the built-in end of the lug (hidden). ![](../graphics/gsk-lugexp-elem.png)
...
The peak Mises stress is on the order of 550 MPa, as shown in [Figure 4--42](ch04s03.md#gsk-lugexp-stresshist). This value is larger than the typical yield strength of steel. Thus, the material would have yielded before experiencing such a large stress. Material nonlinearity is discussed further in [Chapter 10, "Materials](ch10.md).”
