...
A sequence of [Face](pt01ch07pyo05.md) objects or [PartInstance](pt01ch06pyo04.md) objects specifying the domain to search for geometric entities that need to be merged. Entities identified as candidates to be merged may be merged with entities from outside the specified region.
...
A sequence of [PartInstance](pt01ch06pyo04.md) objects or [Region](pt01ch45pyo03.md) objects specifying the part instances or regions from where the native mesh is to be deleted.
...
A sequence of [PartInstance](pt01ch06pyo04.md) objects or Edge objects specifying the part instances or edges from which the seeds are to be deleted.
...
A [Cell](pt01ch07pyo01.md) object specifying the geometric region where the mesh is to be generated. This argument is valid only for native part instances.
...
*geometryConnectingSides*
  ...
  A [Region](pt01ch45pyo03.md) of [Face](pt01ch07pyo05.md) objects specifying the connecting sides of the sweep meshing operation.
...
A sequence of [PartInstance](pt01ch06pyo04.md) objects or [Region](pt01ch45pyo03.md) objects specifying the part instances or regions where the mesh is to be generated.
...
This method returns the [ElemType](pt01ch31pyo03.md) object of a given element shape assigned to a region of the assembly.
...
The return value is dependent on the *attribute* argument.- If *attribute*=ELEM_SHAPE, the return value is a SymbolicConstant specifying the element shape used during meshing. Possible values are: - LINE - QUAD - TRI - QUAD_DOMINATED - HEX - TET - WEDGE - HEX_DOMINATED
...
A sequence or tuple of [PartInstance](pt01ch06pyo04.md) objects or Geometry regions for which mesh statistics should be returned.
...
A [PartInstance](pt01ch06pyo04.md) object specifying the part instance to be queried.
...
The return value is dependent on the value of the *attribute* argument.- If *attribute*=SIZE, the return value is a Float specifying the assigned global element size. If part seeds are not defined, the return value is zero.
...
A sequence of [PartInstance](pt01ch06pyo04.md) objects specifying the part instances to seed.
...
A sequence of [Region](pt01ch45pyo03.md) or [MeshElement](pt01ch31pyo05.md) objects. If you do not specify the *regions* argument, all meshes in the assembly are considered.
