...
Each of the energy quantities can be requested as output and can be plotted as time histories summed over the entire model, particular element sets, individual elements, or as energy density within each element. The variable names associated with the energy quantities summed over the entire model or element sets are as listed in [Table 9--2](ch09s06.md#gxi-table2).
...
In addition, Abaqus/Explicit can produce element-level energy output and energy density output, as listed in [Table 9--3](ch09s06.md#gxi-xpl-table3).
