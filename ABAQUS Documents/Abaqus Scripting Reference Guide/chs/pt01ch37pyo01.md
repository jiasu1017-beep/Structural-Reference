...
A Boolean specifying whether to compress the feature list when copying a Part object. The default value is OFF. If *mirrorPlane* or *scale* is specified, this argument is ignored.

When you compress the feature, list the resulting part will have a single feature. Any datums or sets in the original part will be lost.
...
A [PartInstance](pt01ch06pyo04.md) specifying the base instance from which to cut other instances.

*cuttingInstances*

A sequence of [PartInstance](pt01ch06pyo04.md) objects specifying the instances with which to cut the base instance.
...
A sequence of [PartInstance](pt01ch06pyo04.md) objects specifying the part instances to merge.
...
An [AcisFile](pt01ch37pyo03.md) object specifying a file containing geometry.
...
An Int specifying the desired body to be selected from an ACIS object containing a list of *N* ACIS bodies. Possible values are 1 ![](../graphics/ker_eqn00013.gif) *bodyNum* ![](../graphics/ker_eqn00013.gif) *N*. The default value is 1.
...
### 37.1.7 PartFromInstanceMesh(...)

This method creates a Part object containing the mesh found in the supplied [PartInstance](pt01ch06pyo04.md) objects and places the new Part object in the parts repository.
...
A sequence of [PartInstance](pt01ch06pyo04.md) objects to be used in the creation of the new mesh part. If the *partInstances* argument is omitted, the new Part object contains the mesh of all the part instances in the assembly.
...
### 37.1.10 PartFromNodesAndElements(...)
...
An Int specifying the step number for reading deformed coordinates. ![](../graphics/ker_eqn00410.gif) where ![](../graphics/ker_eqn00411.gif) is the number of available steps. The default value is the last available step. You should specify the *step* argument only when *shape*=DEFORMED.

*frame*

An Int specifying the frame number for reading deformed coordinates. ![](../graphics/ker_eqn00412.gif) where ![](../graphics/ker_eqn00411.gif) is the number of available frames. The default value is the last available frame. You should specify the *frame* argument only when *shape*=DEFORMED.
...
```
The substructure sim file was generated using a version that is different from the current version.
```

### 37.1.14 Part2DGeomFrom2DMesh(...)
...
The optional arguments to `setValues` are the same as the arguments to the [Part](pt01ch37pyo01.md#ker-part-part-pyc) method. In addition, `setValues` has the following optional argument:
...
A [ConstrainedSketch](pt01ch48pyo01.md) object.
...
A sequence of [Face](pt01ch07pyo05.md) objects specifying the regions where thickness will be applied.
...
A sequence of  [Face](pt01ch07pyo05.md) objects whose distance to *faces* argument is used to calculate the thickness along the *faces*. The combination of *topFaces* and *bottomFaces* determines the thickness and the offset of the elements. If *bottomFaces* is not specified then the thickness is twice the distance to the *topFaces*. This argument will be ignored if *thickness* is specified. Either *thickness*, *topFaces*, or *bottomFaces* must be specified.

*bottomFaces*

A sequence of [Face](pt01ch07pyo05.md) objects whose distance to *faces* is used to calculate the thickness along the *faces*. The combination of *topFaces* and *bottomFaces* determines the thickness and the offset of the elements. If *topFaces* is not specified then the thickness is twice the distance to the *bottomFaces*. This argument will be ignored if *thickness* is specified. Either *thickness*, *topFaces*, or *bottomFaces* must be specified.
...
A Part object or a sequence of [Region](pt01ch45pyo03.md) objects specifying the part or regions whose mass properties are to be queried. The whole part is queried by default.
...
### 37.1.35 getFeatureFaces(...)

This method returns a sequence of [Face](pt01ch07pyo05.md) objects that are created by the given feature.
...
Sequence of [Face](pt01ch07pyo05.md) objects.
...
This method returns a sequence of [Edge](pt01ch07pyo03.md) objects that are created by the given feature.
...
Sequence of [Edge](pt01ch07pyo03.md) objects.
...
### 37.1.37 getFeatureCells(...)

This method returns a sequence of [Cell](pt01ch07pyo01.md) objects that are created by the given feature.
...
Sequence of [Cell](pt01ch07pyo01.md) objects.
...
### 37.1.38 getFeatureVertices(...)

This method returns a sequence of [Vertex](pt01ch07pyo15.md) objects that are created by the given feature.
...
Sequence of [Vertex](pt01ch07pyo15.md) objects.
...
### 37.1.41 projectEdgesOntoSketch(...)

This method projects the selected edges of a part onto the specified [ConstrainedSketch](pt01ch48pyo01.md) object. The edges appear as sketch geometry after projection. If the plane of projection is not parallel to the specified edge, the resultant sketch geometry may be of a different type. For example, a circular edge can be projected as an ellipse or a line depending on the angle of the plane of projection. By default, the projected edge will be constrained to the background geometry. You can remove this constraint by setting *constrainToBackground* to False.

**Required arguments**

*sketch*

The [ConstrainedSketch](pt01ch48pyo01.md) object on which the edges are projected.
...
### 37.1.46 queryRegionsMissingSections()
...
A [Region](pt01ch45pyo03.md) object, or None.

**Exceptions**

None.

### 37.1.47 queryDisjointPlyRegions()
...
A  [MeshFace](pt01ch31pyo08.md) object specifying the target region.
...
The Part object has members with the same names and descriptions as the arguments to the [Part](pt01ch37pyo01.md#ker-part-part-pyc) method.
...
A [VertexArray](pt01ch07pyo15.md) object specifying all the vertices in the part.

*ignoredVertices*

An [IgnoredVertexArray](pt01ch07pyo09.md) object specifying all the ignored vertices in the part.

*edges*

An [EdgeArray](pt01ch07pyo03.md) object specifying all the edges in the part.

*ignoredEdges*

An [IgnoredEdgeArray](pt01ch07pyo07.md) object specifying all the ignored edges in the part.

*faces*

A [FaceArray](pt01ch07pyo05.md) object specifying all the faces in the part.

*cells*

A [CellArray](pt01ch07pyo01.md) object specifying all the cells in the part.

*features*

A repository of [Feature](pt01ch20pyo01.md) objects specifying all the features in the part.

*featuresById*

A repository of [Feature](pt01ch20pyo01.md) objects specifying all [Feature](pt01ch20pyo01.md) objects in the part. The [Feature](pt01ch20pyo01.md) objects in the `featuresById` repository are the same as the [Feature](pt01ch20pyo01.md) objects in the `features` repository. However, the key to the objects in the `featuresById` repository is an integer specifying the *ID*, whereas the key to the objects in the `features` repository is a string specifying the *name*.

*datums*

A repository of [Datum](pt01ch15pyo01.md) objects specifying all the datums in the part.

*elements*

A [MeshElementArray](pt01ch31pyo05.md) object specifying all the elements in the part.

*elemFaces*

A repository of [MeshFace](pt01ch31pyo08.md) objects specifying all the element faces in the part. For a given element and a given face index within that element, the corresponding [MeshFace](pt01ch31pyo08.md) object can be retrieved from the repository by using the key calculated as (i*8 + j), where i and j are zero-based element and face indices, respectively.

*elementFaces*

A [MeshFaceArray](pt01ch31pyo08.md) object specifying all the unique element faces in the part.

*nodes*

A [MeshNodeArray](pt01ch31pyo09.md) object specifying all the nodes in the part.

*retainedNodes*

A [MeshNodeArray](pt01ch31pyo09.md) object specifying all the retained nodes in the substructure part.

*sets*

A repository of [Set](pt01ch45pyo04.md) objects specifying for more information, see [Set](pt01ch45pyo04.md).

*allSets*

A repository of [Set](pt01ch45pyo04.md) objects specifying the contents of the *allSets* repository is the same as the contents of the *sets* repository.
...
A repository of [Surface](pt01ch45pyo05.md) objects specifying for more information, see [Surface](pt01ch45pyo05.md).

*allSurfaces*

A repository of [Surface](pt01ch45pyo05.md) objects specifying the contents of the *allSurfaces* repository is the same as the contents of the *surfaces* repository.
...
A repository of [Skin](pt01ch45pyo06.md) objects specifying the skins created on the part.

*stringers*

A repository of [Stringer](pt01ch45pyo07.md) objects specifying the stringers created on the part.
