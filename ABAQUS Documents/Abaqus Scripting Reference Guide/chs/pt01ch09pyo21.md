# 9.21 ElectricPotentialBC 对象

ElectricPotentialBC 对象存储电势边界条件的数据。

ElectricPotentialBC 对象派生自 [BoundaryCondition](pt01ch09pyo01.md) 对象。
...
Float，指定电势大小。默认值为 0。如果 *distributionType*=USER_DEFINED，则 *magnitude* 参数是可选的。
...
此方法修改创建该对象的步骤中现有 ElectricPotentialBC 对象的数据。
...
`setValues` 的可选参数与 [ElectricPotentialBC](pt01ch09pyo21.md#ker-electricpotentialbc-electricpotentialbc-pyc) 方法的参数相同，但 *name* 和 *createStepName* 参数除外。
...
此方法修改指定步骤中现有 ElectricPotentialBC 对象的传播数据。
...
Float 或 SymbolicConstant FREED，指定电势大小。
...
ElectricPotentialBC 对象可以具有以下成员：
