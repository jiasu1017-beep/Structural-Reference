# 49.17 InitialStep 对象

InitialStep 对象是一个占位符，您无法创建、删除或修改它。每个模型默认都存在 InitialStep 对象，用于允许在模型中定义预先存在的边界条件相互作用。

InitialStep 对象派生于 [AnalysisStep](pt01ch49pyo02.md) 对象。
