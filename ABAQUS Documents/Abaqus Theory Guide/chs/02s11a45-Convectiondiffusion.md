...
流体以速度![](../graphics/stm_eqn00427)流动的连续体的热平衡方程为
...
边界条件为在部分表面![](../graphics/stm_eqn02318)上规定温度，在其余表面![](../graphics/stm_eqn02320)上进入域的每单位面积热通量被规定或由对流和/或辐射条件定义。例如，流体对流单元和固体单元之间的边界层可以通过DINTER*x*型单元建模。热平衡方程中的边界项定义
...
这些方程通过使用一次等参单元相对于位置离散化。流体速度![](../graphics/stm_eqn00427)假设为已知的。（Abaqus实际上要求定义流体每单位面积的质量流率，因为这通常对用户更方便。速度由质量流率和流体密度计算。）

时间离散化从时间*t*的已知解生成时间![](../graphics/stm_eqn00438)的解。

温度的插值![](../graphics/stm_eqn01111)在单元和时间里增量上定义为
...
![](../graphics/stm_eqn02330.gif)其中![](../graphics/stm_eqn01256)是单元中的局部Peclet数，*C*是局部Courant数，定义为
...
设![](../graphics/stm_eqn02332)是穿过单元中心点的![](../graphics/stm_eqn00904)等参线。单元中心点处流体速度向量方向上![](../graphics/stm_eqn02332)的投影为
...
![](../graphics/stm_eqn02336)仅能对用于离散化![](../graphics/stm_eqn02337)的加权函数的连续部分执行：否则，不能保证单元之间热通量的连续性。为方便起见，我们将加权的连续部分写为
...
我们现在从时间*t*到![](../graphics/stm_eqn00438)对这个方程进行积分，以提供增量的平均平衡陈述。我们使用结果
