...
![](../graphics/stm_eqn06481)其中![](../graphics/stm_eqn06482)是材料的初始（未损伤）弹性刚度；![](../graphics/stm_eqn06483)是退化弹性刚度；*d*是标量刚度退化变量，可以取从零（未损伤材料）到一（完全损伤材料）的值。与混凝土破坏机制（裂缝和压碎）相关的损伤导致弹性刚度降低。在标量损伤理论范围内，刚度退化是各向同性的，由单个退化变量*d*表征。遵循连续体损伤力学的通常概念，有效应力定义为

![](../graphics/stm_eqn06484)Cauchy应力通过标量退化关系与有效应力相关：

![](../graphics/stm_eqn06485)对于材料的任何给定横截面，系数![](../graphics/stm_eqn06486)表示有效承载面积（即，整体面积减去损伤面积）与整体横截面积之比。在没有损伤的情况下，![](../graphics/stm_eqn06487)，有效应力![](../graphics/stm_eqn06488)等价于Cauchy应力![](../graphics/stm_eqn00033)。然而，当损伤发生时，有效应力比Cauchy应力更具代表性，因为是有效应力面积在抵抗外部载荷。因此，方便的是将塑性问题以有效应力表示。如后面讨论的，退化变量的演化由一组硬化变量![](../graphics/stm_eqn06489)和有效应力控制；即，![](../graphics/stm_eqn06490)。
### 硬化变量
...
![](../graphics/stm_eqn06495)屈服函数的具体形式在本节后面描述。
### 流动规则
...
在单轴循环加载条件下，退化机制相当复杂，涉及先前形成的微裂缝的张开和闭合，以及它们的相互作用。实验上，观察到在单轴循环试验中当载荷改变符号时弹性刚度有一些恢复。刚度恢复效应，也称为"单侧效应"，是混凝土在循环加载下行为的一个重要方面。效应通常在载荷从拉伸变为压缩时更明显，导致拉伸裂缝闭合，从而恢复压缩刚度。

混凝土损伤塑性模型假定弹性模量的降低以标量退化变量*d*给出，如

![](../graphics/stm_eqn06518)其中![](../graphics/stm_eqn06513)是材料的初始（未损伤）模量。

该表达式在循环的拉伸侧（![](../graphics/stm_eqn06519）和压缩侧（![](../graphics/stm_eqn06520）都适用。刚度降低变量*d*是应力状态和单轴损伤变量![](../graphics/stm_eqn06510)和![](../graphics/stm_eqn06511)的函数。对于单轴循环条件，Abaqus假定

![](../graphics/stm_eqn06521)其中![](../graphics/stm_eqn06522)和![](../graphics/stm_eqn06523)是引入的表示与应力反向相关刚度恢复效应的应力状态函数。它们根据以下定义
...
![](../graphics/stm_eqn06555)标量退化变量*d*的定义必须与单轴单调响应（![](../graphics/stm_eqn06510)和![](../graphics/stm_eqn06511)）一致，而且还应捕捉循环加载下退化机制的复杂性。对于一般多轴应力条件，Abaqus假定
...
![](../graphics/stm_eqn06560)其中![](../graphics/stm_eqn00904)和![](../graphics/stm_eqn01256)是无量纲材料常数；

![](../graphics/stm_eqn06561)是有效静水压力；

![](../graphics/stm_eqn06562)是Mises等效有效应力；

![](../graphics/stm_eqn06563)是有效应力张量![](../graphics/stm_eqn06488)的偏量部分；且![](../graphics/stm_eqn06564)是![](../graphics/stm_eqn06488)的代数最大特征值。函数![](../graphics/stm_eqn06565)给出为

![](../graphics/stm_eqn06566)其中![](../graphics/stm_eqn06515)和![](../graphics/stm_eqn06516)分别是有效拉伸和压缩内聚应力。

在双轴压缩中，当![](../graphics/stm_eqn06567)，[公式4.5.2-11](04s05a120.md)简化为著名的Drucker-Prager屈服条件。系数![](../graphics/stm_eqn00904)可以从初始等双轴和单轴压缩屈服应力![](../graphics/stm_eqn06568)和![](../graphics/stm_eqn06569)确定为

![](../graphics/stm_eqn06570)混凝土的典型实验值。![](../graphics/stm_eqn06571)的比值在1.10到1.16之间，产生![](../graphics/stm_eqn00904)在0.08到0.12之间的值（[Lubliner等，1989](07s01a01-References.md)）。

系数![](../graphics/stm_eqn01256)仅在三轴压缩应力状态下进入屈服函数，当![](../graphics/stm_eqn06572)时。该系数可以通过比较拉伸和压缩子午线上的屈服条件来确定。*拉伸子午线*（TM）定义为满足条件![](../graphics/stm_eqn06573)的应力状态轨迹，*压缩子午线*（CM）是满足条件![](../graphics/stm_eqn06574)的应力状态轨迹，其中![](../graphics/stm_eqn06575)、![](../graphics/stm_eqn06576)和![](../graphics/stm_eqn06577)是有效应力张量的特征值。可以容易地表明，沿拉伸和压缩子午线分别有![](../graphics/stm_eqn06578)和![](../graphics/stm_eqn06579)。当![](../graphics/stm_eqn06580)时，相应的屈服条件为

![](../graphics/stm_eqn06581)

![](../graphics/stm_eqn06582)设对于任何给定的静水压力值![](../graphics/stm_eqn06584)与![](../graphics/stm_eqn06580)，![](../graphics/stm_eqn06583)；则

![](../graphics/stm_eqn06585)![](../graphics/stm_eqn06586)是常数这一事实似乎没有被实验证据反驳（[Lubliner等，1989](07s01a01-References.md)）。因此，系数![](../graphics/stm_eqn01256)计算为

![](../graphics/stm_eqn06587)值为![](../graphics/stm_eqn06588)（对混凝土典型）给出![](../graphics/stm_eqn06589)

如果![](../graphics/stm_eqn06590)，则沿拉伸和压缩子午线的屈服条件简化为
...
表现出软化行为和刚度退化的材料模型通常在隐式分析程序中导致严重的收敛困难。可以通过使用本构方程的粘塑性正则化来克服一些这些收敛困难。混凝土损伤塑性模型可以使用粘塑性进行正则化，因此允许应力位于屈服面之外。我们使用Duvaut-Lions正则化的推广，其中粘塑性应变率张量![](../graphics/stm_eqn06598定义为
