...
AnnealStep 对象通过将速度，所有适当的状态变量设置为零来对结构进行退火。

AnnealStep 对象派生于 [AnalysisStep](pt01ch49pyo02.md) 对象。
...
一个 Float，指定退火后的参考温度。默认值为退火完成后模型中所有节点的当前温度。
...
`setValues` 的可选参数与 [AnnealStep](pt01ch49pyo03.md#ker-annealstep-annealstep-pyc) 方法的参数相同，但 *name*、*previous* 和 *maintainAttributes* 参数除外。
