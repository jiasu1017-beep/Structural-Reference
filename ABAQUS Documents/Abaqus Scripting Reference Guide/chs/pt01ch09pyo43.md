# 9.43 TemperatureBC 对象

TemperatureBC 对象存储温度边界条件的数据。

TemperatureBC 对象派生自 [BoundaryCondition](pt01ch09pyo01.md) 对象。
...
Int 的序列，指定边界条件所应用到的自由度。默认值为 (11,)。
...
此方法修改创建该对象的步骤中现有 TemperatureBC 对象的数据。
...
`setValues` 的可选参数与 [TemperatureBC](pt01ch09pyo43.md#ker-temperaturebc-temperaturebc-pyc) 方法的参数相同，但 *name* 和 *createStepName* 参数除外。
...
此方法修改指定步骤中现有 TemperatureBC 对象的传播数据。
...
Float 或 SymbolicConstant FREED，指定温度大小。
