...
当驱动节点位于中心区域内时，所有位移自由度都被驱动。对于几何线性分析，这些规定位移从图像节点的位移和旋转获得为

![](../graphics/stm_eqn02562)其中![](../graphics/stm_eqn02563)是驱动节点*A*的规定位移，![](../graphics/stm_eqn02564)和![](../graphics/stm_eqn02565)是图像节点的插值位移和旋转，![](../graphics/stm_eqn00424)是连接图像节点到驱动节点的向量：

![](../graphics/stm_eqn02566)

对于大位移分析，必须考虑有限旋转。[方程2.15.1-1](02s15a51.md)的有限旋转等效为

![](../graphics/stm_eqn02567)其中![](../graphics/stm_eqn00162)是"旋转变量"第1.3.1节中定义的旋转矩阵；![](../graphics/stm_eqn00064)是单位张量；![](../graphics/stm_eqn02568)是连接图像节点到驱动节点的当前配置中的旋转向量：

![](../graphics/stm_eqn02569)

对于中心区域外的驱动节点，仅平行于壳中面的位移分量被驱动。对于几何线性情况，这导致约束

![](../graphics/stm_eqn02570)其中![](../graphics/stm_eqn02571)和![](../graphics/stm_eqn02572)是垂直于![](../graphics/stm_eqn00424)的两个（单位）向量。几何非线性情况的等效表达式为
...
小位移理论的一般过程到一般过程：中心区域内使用[方程2.15.1-1](02s15a51.md)，中心区域外使用[方程2.15.1-3](02s15a51.md)。

大位移理论的一般过程到一般过程：中心区域内使用[方程2.15.1-2](02s15a51.md)，中心区域外使用[方程2.15.1-4](02s15a51.md)。

小位移理论的一般过程到线性摄动过程：中心区域内

![](../graphics/stm_eqn02576)中心区域外

![](../graphics/stm_eqn02577)；![](../graphics/stm_eqn02578)表示子模型中的基态。

大位移理论的一般过程到线性摄动过程：中心区域内

![](../graphics/stm_eqn02579)中心区域外
...
小位移理论的线性摄动过程到一般过程：中心区域内

![](../graphics/stm_eqn02583)中心区域外

![](../graphics/stm_eqn02584)；![](../graphics/stm_eqn02578)表示子模型中的基态。

大位移理论的线性摄动过程到一般过程：中心区域内

![](../graphics/stm_eqn02585)中心区域外
...
小位移理论的线性摄动过程到线性摄动过程：中心区域内

![](../graphics/stm_eqn02588)中心区域外

![](../graphics/stm_eqn02589)。

大位移理论的线性摄动过程到线性摄动过程：中心区域内

![](../graphics/stm_eqn02590)中心区域外
...
图2.15.1-4 对位于单个全局模型单元内的驱动积分点的驱动应力有贡献的全局单元范围。

![](../graphics/stmsmoothingpatch-nls.png)图中中心处驱动积分点（那些在深色阴影单元编号5内的）的应力从全局模型节点A、B、C和D插值。该图通过单元1、4、5和8的浅色阴影说明了全局单元对节点A处计算的应力的贡献。考虑节点B、C和D的额外贡献，因此对位于全局单元5内的驱动积分点有贡献的单元完整集合是图中显示的所有单元。
### 表面牵引力确定
